...
public class SecurityConfig {
	 ...
	 * authorities("ADMIN") .build(); UserDetails userAbir =
	 * User.withUsername("abir").password(passwordEncoder.encode("123"))
	 * .authorities("AGENT", "USER").build(); UserDetails user1 =
	 * User.withUsername("user1").password(passwordEncoder.encode("123")).
	 * authorities("USER") .build(); UserDetails user2 =
	 * User.withUsername("sougui").password(passwordEncoder.encode("sougui")).
	 * authorities("USER") .build();
	 * 
	 * return new InMemoryUserDetailsManager(admin, userAbir, user1, user2); }
	 */
